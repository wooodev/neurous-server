...
public class ContentGenerationService {
    ...
    */
    ...
    private int generate3LevelsForOneArticle(NewsArticle a) {
        ...
        saved += generateAndSave(a, ContentDifficulty.BEGINNER);
        saved += generateAndSave(a, ContentDifficulty.INTERMEDIATE);
        saved += generateAndSave(a, ContentDifficulty.ADVANCED);

        return saved;
    }


    private int generateAndSave(NewsArticle a, ContentDifficulty diff) {

        if (contentRepository.existsByNewsArticleIdAndContentDiff(a.getNewsArticleId(), diff.name())) {
            return 0;
        }
        ...
        Content entity = Content.builder()
                .title(a.getTitle())
                .content(body)
                .contentCategory(a.getCategory().name())
                .contentDiff(diff.name())
                .imageUrl(null)
    ...
    private String callClova(String userPrompt, ContentDifficulty diff) {
        String system = """
    ...
    private int maxTokensByDifficulty(ContentDifficulty diff) {
        return switch (diff) {
            case BEGINNER -> 300;
    ...
    private String buildPrompt(NewsArticle a, ContentDifficulty diff) {
    ...
    private boolean isValid(String text, ContentDifficulty diff) {
